...
# Fill missing satisfaction_rating with median
...
# Fill missing loyalty_status with mode (most common tier)
...
# Use max(last_purchase) as "today" reference
...
df_no_duplicates["purchase_frequency_category"] = pd.cut(
    df_no_duplicates["total_purchases"], bins=[-1,4,9,np.inf], labels=["Low","Medium","High"]
)
...
# -----------------------------
# TODO 8: Insights
df_unique = df_no_missing.drop_duplicates(subset="customer_id", keep="first").copy()

df_unique["total_spent"] = pd.to_numeric(df_unique["total_spent"], errors="coerce")

# Step 3: Compute average spent by loyalty including all loyalty tiers (including "Unknown")
avg_spent_by_loyalty = (
    df_unique.groupby("loyalty_status")["total_spent"]
    .mean()
    .sort_values(ascending=False)
)

# Step 4: Revenue by preferred category
df_unique["preferred_category"] = df_unique["preferred_category"].str.title()
category_revenue = (
    df_unique.groupby("preferred_category")["total_spent"]
    .sum()
    .sort_values(ascending=False)
)

# Step 5: Correlation between satisfaction rating and total spent
satisfaction_spend_corr = df_unique["satisfaction_rating"].corr(df_unique["total_spent"])
...
print("- Revenue by Loyalty (valid tiers only):")
